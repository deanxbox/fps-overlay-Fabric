 ...
 * from the Minecraft client. Adapted for MC 1.21.1.
 */
public final class MetricProvider {
    ...
    public static TickData fetchTickData(Minecraft client) {
        ...
        if (server != null) {
            // getAverageTickTimeNanos() exists in NeoForge 21.1.x; convert to ms
    ...
    public static ChunkData fetchChunks(Minecraft client) {
        ...
        // No reliable single-call API for completed chunks in 1.21.1; fall back to visible
        completed = visible;
    ...
    public static LocationData fetchLocation(Minecraft client) {
        ...
        try {
            // In MC 1.21.1, ResourceKey uses .location() not .identifier()
            biome = client.level.getBiome(pos)
                    .unwrapKey()
                    .map(key -> Component.translatable(key.location().toLanguageKey("biome")).getString())
                    .orElse("Unknown");
        } catch (Exception ignored) {
        }
