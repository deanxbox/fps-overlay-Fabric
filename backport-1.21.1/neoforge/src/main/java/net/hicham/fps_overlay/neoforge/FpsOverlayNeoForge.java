...
public class FpsOverlayNeoForge {
    // In 1.21.1, KeyMapping category is a plain String translation key
    private static final String KEY_CATEGORY = "key.categories." + FpsOverlayMod.MOD_ID + ".keys";
    ...
    public FpsOverlayNeoForge(IEventBus modEventBus, ModContainer modContainer) {
        if (FMLEnvironment.dist == Dist.CLIENT) {
            ...
            modContainer.registerExtensionPoint(IConfigScreenFactory.class,
                    (client, parent) -> net.hicham.fps_overlay.ConfigScreenFactory.createConfigScreen(parent));
        }
    }

    private void onLoggingIn(net.neoforged.neoforge.client.event.ClientPlayerNetworkEvent.LoggingIn event) {
        net.hicham.fps_overlay.ServerTickMetrics.onJoinServer(event.getConnection());
    }
